...
var width = 800;//parseInt(d3.select("#graph").style("width")) - margin.left - margin.right,
    height = 500;
// create svg
var svg_clt = d3.select("#clt").append("svg")
  .attr("width", "100%")
  .attr("height", "100%")
...
// create histogram
var histogram = d3.histogram().domain(x_scale_clt.domain()).thresholds(x_scale_clt.ticks(bins))//.frequency(false);
...
function draw_histogram() {
  ...
  // update scale
  var ymax = d3.max(data.map(function(d) { return d.length; }));
  ...
  // update bars
  bar.select("rect")
    .attr("x", function(d) { return x_scale_clt(d.x0) + 1; })
    .attr("width", function(d) { return x_scale_clt(data[0].x1) - x_scale_clt(data[0].x0) - 1; })//x_scale_clt(data[0].x1) - x_scale_clt(data[0].x0) - 1)
  .transition().duration(250)
    .attr("y", function(d) { return 3*y1 - y_scale_clt(d.length*bins); })
    .attr("height", function(d) { return y_scale_clt(d.length*bins); });
  bar.select("text")
    .attr("x", function(d) { return x_scale_clt(d.x0 + 1/(2*bins)); })
    .text(function(d) { return d.length > 0 ? d3.format(".0%")(d.length / counts.length) : ""; });
  // exit bars
  bar.exit().remove();
};
